...
{	
	...
	// 获得SD卡中文件数量
	// cnt = mf_filenums(SD_FILE_DIR);
	// printf("destination dir:%s num:%d\r\n", SD_FILE_DIR, cnt);
	...
	if(f_opendir(&dir,folder_d) != FR_OK)	printf("res1:%d\r\n",f_mkdir(folder_d));  // 创建目录

	if(f_opendir(&dir,USB_FILE_DIR_CR) != FR_OK) 
	{
		printf("res2:%d\r\n",f_mkdir(USB_FILE_DIR_CR));  // 创建目录
	}
	
	res = f_opendir(&dir,folder_s);  // 打开目录
	// 成功打开目录
	if (res == FR_OK)  
	{
		while(1)
...
//			if(err != MY_MQTT_ERR_NONE) {printf("mysend_picture err:%d\r\n", err);continue;}
//			
//			// 移动文件
//			res = f_rename(path,chg_path);
//			if(res != FR_OK)
