...
void jsB_init(js_State *J) //0x7B68
{
	/* Create the prototype objects here, before the constructors */
	J->Object_prototype = jsV_newobject(J, JS_COBJECT, NULL); //0x7BA8
	J->Array_prototype = jsV_newobject(J, JS_CARRAY, J->Object_prototype); //0x7BE8
	J->Function_prototype = jsV_newobject(J, JS_CCFUNCTION, J->Object_prototype);//0x7C28
	J->Boolean_prototype = jsV_newobject(J, JS_CBOOLEAN, J->Object_prototype);//0x7C68
	J->Number_prototype = jsV_newobject(J, JS_CNUMBER, J->Object_prototype);//0x7CA8
	J->String_prototype = jsV_newobject(J, JS_CSTRING, J->Object_prototype);//0x7CE8
	J->RegExp_prototype = jsV_newobject(J, JS_COBJECT, J->Object_prototype);//0x7D28
	//J->Date_prototype = jsV_newobject(J, JS_CDATE, J->Object_prototype);

	/* All the native error types */
	J->Error_prototype = jsV_newobject(J, JS_CERROR, J->Object_prototype);//0x7D68
	J->EvalError_prototype = jsV_newobject(J, JS_CERROR, J->Error_prototype);//0x7DA8
	J->RangeError_prototype = jsV_newobject(J, JS_CERROR, J->Error_prototype);
	J->ReferenceError_prototype = jsV_newobject(J, JS_CERROR, J->Error_prototype);//0x7DE8
	J->SyntaxError_prototype = jsV_newobject(J, JS_CERROR, J->Error_prototype);//0x7E28
	J->TypeError_prototype = jsV_newobject(J, JS_CERROR, J->Error_prototype);//0x7E68
	...
	jsB_initobject(J);//0x9BD7
	jsB_initarray(J);//0xBB63
	jsB_initfunction(J);//0xC24C
	jsB_initboolean(J);//0xC640
	jsB_initnumber(J);//0xD189
	jsB_initstring(J);//0xEF77
	jsB_initregexp(J);//0xF4EC
	//jsB_initdate(J);
	jsB_initerror(J);
	jsB_initmath(J);
	jsB_initjson(J);
